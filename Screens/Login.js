...
const PeerResetAction = StackActions.reset({
  index: 0,                       
  actions: [NavigationActions.navigate({ routeName: 'PeerHomescreen'})],
});

const DistributorResetAction = StackActions.reset({
  index: 0,                       
  actions: [NavigationActions.navigate({ routeName: 'DistributorHomescreen'})],
});

const RetailerResetAction = StackActions.reset({
  index: 0,                       
  actions: [NavigationActions.navigate({ routeName: 'TrackOrders'})],
});

const ShopResetAction = StackActions.reset({
  index: 0,                       
  actions: [NavigationActions.navigate({ routeName: 'AddShop',params: { from: 'login' }})],
});
...
export default class Login extends React.Component {
  ...
  //     await this.requestSmsPermission(); // Ask for SMS permission
...
//                 message: "App needs permission to read SMS for OTP autofill",
...
//         const message = await RNSmsRetriever.startSmsRetriever();
//         console.log("Received SMS: ", message); // Debugging

//         if (message && typeof message === "string") {
//             const otpCode = message.match(/\b\d{4,6}\b/); // Extract 4-6 digit OTP
//             if (otpCode) {
//                 this.setState({ otp: otpCode[0] });
//                 console.log("Extracted OTP: ", otpCode[0]);
//             } else {
//                 console.log("No OTP found in SMS.");
//             }
//         } else {
//             console.log("Received message is invalid:", message);
//         }
//     } catch (error) {
...
// };
    ...
    loginApi(type){
              ...
              // formData.append("peer_code",this.state.peer);
        ...
        <View style={{flex:1,backgroundColor:'#FFF'}}>
              <StatusBar backgroundColor="transparent" translucent={true} barStyle="dark-content" />
                ...
                {!this.state.show_name && <View style={{flex:1}}>
                  ...
                  <TouchableOpacity onPress={()=> this.onLogin()} style={{height:45,width:'100%',backgroundColor:'#F37A20',borderRadius:10,alignSelf:'center',marginTop:30,justifyContent:'center'}}>
                ...
                {this.state.show_otp && <BottomSheet
                  ...
                  <TouchableOpacity onPress={()=> this.verifyOtp()} style={{height:45,width:200,backgroundColor:'#F37A20',borderRadius:30,alignSelf:'center',marginTop:30,justifyContent:'center'}}>
